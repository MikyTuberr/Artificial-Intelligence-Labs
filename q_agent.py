...
class QAgent(Agent):
    ...
    def __init__(self, n_states, n_actions,
        ...
        self.eps_decrement = 1.5e-5     # wartość, o którą zmniejsza się epsilon po każdym kroku
    ...
    def update_action_policy(self) -> None:
        ...
        if self.epsilon - self.eps_decrement <= self.eps_min:
    ...
    def learn(self, state: State, action: Action, reward: float, new_state: State, done: bool) -> None:
        ...
        o = reward + self.gamma * np.max(self.q_table[new_state, :]) - self.q_table[state, action]
        self.q_table[state, action] += self.lr * o
